...
import com.api.medicine.application.use_cases.GetPredictionUseCase;
...
public class PharmacyController {
    ...
    public PharmacyController(AddStockUseCase addStockUseCase, ATMRepository atmRepository,GetPredictionUseCase getPredictionUseCase) {
        this.addStockUseCase = addStockUseCase;
        this.atmRepository = atmRepository;
        this.getPredictionUseCase = getPredictionUseCase;
    }
    ...
    /**
     * GET /api/pharmacy/predictions
     * Python ML servisinden AI tabanlı satış tahminlerini çeker.
     */
    @GetMapping("/predictions") // <<< YENI ENDPOINT
    public ResponseEntity<?> getPredictions() {
        Map<String, Object> predictions = getPredictionUseCase.execute();

        if (predictions.containsKey("error")) {
            return ResponseEntity.status(503).body(predictions); // 503 Service Unavailable
        }
