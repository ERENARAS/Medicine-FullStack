<template>
  <div class="reports-container">
    ...
    <div class="stats-grid">
      ...
      <!-- AI Prediction Summary (Dynamic) -->
      <v-card class="stat-card elevation-2">
        <div class="stat-icon-wrapper purple-tint">
          <v-icon color="#9C27B0">mdi-auto-fix</v-icon>
        </div>
        <div class="stat-info">
          <p class="stat-label">AI Tahmin Güvenilirliği</p>
          <h3 class="stat-value">{{ averageConfidence }}%</h3>
        </div>
      </v-card>
    </div>
    ...
    <!-- AI Predictions & Low Stock Alerts Row -->
    <div class="bottom-grid">
        <!-- AI Predictions Table -->
        <v-card class="table-card elevation-2">
            <div class="card-header">
                <h3>AI Satış Tahminleri (Gelecek Ay)</h3>
                <p>Yapay zeka modellerinin satış tahminleri</p>
            </div>
             <div v-if="isLoadingPredictions" class="loader">
                <v-progress-circular indeterminate color="#a2d6b8"></v-progress-circular>
            </div>
            <div v-else-if="predictionError" class="error-msg">{{ predictionError }}</div>
            <v-table v-else>
                <thead>
                    <tr>
                        <th class="text-left">İlaç Adı</th>
                        <th class="text-center">Tahmin (Adet)</th>
                        <th class="text-center">Güven Skoru</th>
                        <th class="text-right">Durum</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="data in filteredPredictions" :key="data.name">
                        <td>{{ data.name }}</td>
                        <td class="text-center">{{ data.next_month_demand }}</td>
                        <td class="text-center">
                            <v-chip :color="getConfidenceColor(data.confidence)" size="small" variant="flat">
                                %{{ (data.confidence * 100).toFixed(0) }}
                            </v-chip>
                        </td>
                         <td class="text-right">
                             <span class="trend-icon up" v-if="data.next_month_demand > 50"><v-icon size="small">mdi-trending-up</v-icon> Yüksek</span>
                             <span class="trend-icon" v-else><v-icon size="small">mdi-trending-neutral</v-icon> Normal</span>
                         </td>
                    </tr>
                </tbody>
            </v-table>
...
const isLoadingPredictions = ref(true);
...
const fetchPredictions = async () => {
    ...
    try {
        const response = await axios.get(`${API_BASE_URL}/api/pharmacy/predictions`);
        predictions.value = response.data;
    } catch (error) {
        console.error("Tahmin hatası:", error);
        predictionError.value = "Tahmin verisi alınamadı";
    } finally {
...
const totalSales = computed(() => {
    const total = Object.values(predictions.value).reduce((acc, curr) => acc + (curr.next_month_demand || 0), 0);
    return total >= 1000 ? (total / 1000).toFixed(1) : total;
});
...
const filteredPredictions = computed(() => {
    return Object.entries(predictions.value)
        ...
        .filter(item => item.next_month_demand > 0)
        .sort((a, b) => b.next_month_demand - a.next_month_demand)
        .slice(0, 7);
});

const averageConfidence = computed(() => {
    const values = filteredPredictions.value;
...
.action-btn {
  color: #fff !important; /* Design shows dark bg so text white? No, user has light dashboard. 
                             Using dark text on light. Or specific colors. */
...
.stat-card {
  ...
  align-items: center;
  gap: 15px;
  background-color: #fff; /* or dark based on image */
}
